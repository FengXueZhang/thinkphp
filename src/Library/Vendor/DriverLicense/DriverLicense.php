...
class DriverLicense
{
	private static $appKey = "23476113";
    private static $appSecret = "49de4273fcd8c0738040019d652ae909";
	//协议(http或https)://域名:端口，注意必须有http://或https://
    private static $host = "https://dm-53.data.aliyun.com";
	...
	*/
	public function doPostString($base_data, $APPCODE) {
		//域名后、query前的部分
		$path = "/rest/160601/ocr/ocr_vehicle.json";
		$request = new HttpRequest($this::$host, $path, HttpMethod::POST, $this::$appKey, $this::$appSecret);
		//传入内容是json格式的字符串
		$bodyContent = '{"inputs":[{"image":{"dataType":50,"dataValue":"'. $base_data .'"}}]}';
	...
	*/
	public function doPostStream() {
		...
		$bytes = array();
		//传入内容是json格式的字符串
		$bodyContent = "{\"inputs\": [{\"image\": {\"dataType\": 50,\"dataValue\": \"base64_image_string\"}]}";
